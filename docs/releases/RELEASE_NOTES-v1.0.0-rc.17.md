...
- Bounds the LaunchServices helper during quarantined first launch, so the CLI cannot wait indefinitely after Okra.app has started.
...
- Hardens the release lane against stale ad-hoc development containers and unbounded launch helpers while retaining Developer ID, hardened-runtime, notarization, Gatekeeper, quarantined LaunchServices, and production-DMG checks; mounted installers use the canonical `okraPDF` volume label.
...
Then run `okra status`, `okra chandra document.pdf`, or `okra presidio document.pdf`. The reserved RC.13 through RC.16 tags produced no public release assets; RC.17 is the first published candidate containing the app-owned CLI and source/facet workspace.
