...
struct LocalProviderSetupTests {
    ...
    @Test("Chandra OCR 2 manifest pins every required runtime artifact")
    func chandraOCRModelManifest() {
        #expect(ChandraOCRModelManifest.artifacts.count == 9)
        #expect(ChandraOCRModelManifest.totalBytes == 5_156_826_785)
        #expect(ChandraOCRModelManifest.artifacts.allSatisfy { $0.sha256.count == 64 })
        #expect(Set(ChandraOCRModelManifest.artifacts.map(\.path)).count == 9)
        #expect(
            ChandraOCRModelManifest.artifacts.contains {
                $0.path == "model-00001-of-00002.safetensors"
                    && $0.size == 5_000_801_612
                    && $0.sha256
                        == "970ad5f1f22a2ce95a4ce69fc291e73d4a9b11c24cc8bcffe6a76ad49a4ffce9"
            }
        )
    }

    @Test("Chandra OCR 2 setup records the accepted model-license revision")
    func chandraOCRSetupRecordsLicenseAcceptance() async throws {
        let workspace = try TestWorkspace(prefix: "okra-chandra-license")
        ...
        coordinator.installSelectedProvider()
        try await waitUntil("Chandra OCR simulated setup to finish") {
            coordinator.isInstalling == false
        }
        ...
        #expect(
            ...
            ) == ChandraOCRModelManifest.package.licenseRevision
        )
    }
