...
@Suite("Workspace plugin rows")
struct WorkspacePluginItemTests {
    ...
    func readinessMapping() {
        #expect(WorkspacePluginItem.readiness(for: .ready) == .ready)
        #expect(WorkspacePluginItem.readiness(for: .simulated("Simulated")) == .simulated)
        #expect(WorkspacePluginItem.readiness(for: .setupRequired("Download first")) == .needsSetup)
        #expect(WorkspacePluginItem.readiness(for: .unavailable("No GPU")) == .unavailable)
        // A provider we have not probed yet is not offered as runnable.
        #expect(WorkspacePluginItem.readiness(for: nil) == .unavailable)
    }

    @Test("Rows carry the availability line, the badge, and the selection")
    func rowsCarryStatusBadgeAndSelection() {
        let items = WorkspacePluginItem.items(
