...
/// The Plugins section of the workspace sidebar.
///
/// Choosing an engine and preparing it lives here; running it lives in Extract
/// on the trailing side. The selected plugin expands in place to show whatever
/// it still needs — a download, an Ollama model — so setup happens next to the
/// thing being set up.
struct WorkspacePluginsSectionView: View {
    @ObservedObject var coordinator: LocalProcessingCoordinator

    private var items: [WorkspacePluginItem] {
        WorkspacePluginItem.items(
            ...
            ForEach(items) { item in
                VStack(alignment: .leading, spacing: WorkspaceTheme.compactSpacing) {
                    Button {
                        ...
                        WorkspacePluginRowView(item: item)
                    }
                    .buttonStyle(.plain)
    ...
    @ViewBuilder
    private func selectedPluginDetail(for item: WorkspacePluginItem) -> some View {
...
/// A single plugin row: name, why you'd pick it, and whether it can run now.
struct WorkspacePluginRowView: View {
    let item: WorkspacePluginItem
