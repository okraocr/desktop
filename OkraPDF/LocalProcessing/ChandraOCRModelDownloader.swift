...
struct ChandraOCRModelDownloader: ChandraOCRModelDownloading {
    func downloadModel(
        to modelURL: URL,
        ...
        for artifact in ChandraOCRModelManifest.artifacts {
        ...
        for artifact in ChandraOCRModelManifest.artifacts {
            ...
            let digest = try FileSHA256.digest(
                ...
                let fraction = Double(verifiedBeforeArtifact + bytesRead)
                    / Double(ChandraOCRModelManifest.totalBytes)
                progress(
                    LocalProviderSetupProgress(
                        phase: .verifying,
                        fraction: fraction,
                        message: "Verifying Chandra OCR 2…"
                    )
                )
            }
    ...
    private func download(
        ...
        guard let downloadURL = ChandraOCRModelManifest.downloadURL(for: artifact) else {
    ...
    private func reportDownloadProgress(
        ...
        progress(
            LocalProviderSetupProgress(
                phase: .downloadingModel,
                fraction: Double(completedBytes) / Double(ChandraOCRModelManifest.totalBytes),
