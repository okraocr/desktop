#!/usr/bin/env python3
"""Chandra OCR 2 worker for okraPDF on Windows.

Port of apps/desktop/OkraPDF/ProviderScripts/chandra-ocr-worker.py (macOS,
mlx-vlm) to transformers/torch. Two modes:

  serve   — loopback HTTP inference server (client-server contract with the
            Go backend). Announces {"port": N} on stdout, then:
              GET  /health -> {"ok": true, "loaded": bool, "simulation": bool}
              POST /page   -> StructuredExtractionPage JSON
                             {"page_number", "image_path", "image_file"}
              POST /shutdown

  (legacy) same CLI as macOS:
    --model --output --page-output-directory --page-progress --title --images

The OCR_LAYOUT_PROMPT, block parsing, normalization, and diagnostics are kept
verbatim from the macOS worker; the model was trained against these prompts.
"""
...
# ---------------------------------------------------------------------------
# Prompts and label tables (verbatim from the macOS worker — do NOT paraphrase)
# ---------------------------------------------------------------------------
...
# Stop generation once the exact same token chunk repeats back to back.
...
MODEL_REPOSITORY = "datalab-to/chandra-ocr-2"
MODEL_REVISION = "af93b47dba1b47b6640c86ccf487ed2260ab9a09"


def runtime_lock_version() -> str:
    """Provenance for the actual execution track (set after model load)."""
    try:
        import torch

        if torch.cuda.is_available():
            return "python>=3.10|torch==2.13.0+cu126|torchvision==0.28.0+cu126|transformers==5.15.0|huggingface-hub==1.24.0|pillow>=11|bitsandbytes>=0.50|accelerate>=1.14|v4-cuda"
    except Exception:
        pass
    return "python>=3.10|torch==2.13.0+cpu|torchvision==0.28.0+cpu|transformers==5.15.0|huggingface-hub==1.24.0|pillow>=11|accelerate>=1.14|v4-cpu"
...
def simulated_layout(image_name: str) -> str:
    return (
        ...
        f"<p>Simulated local OCR for <code>{image_name}</code>.</p></div>"
    )


# ---------------------------------------------------------------------------
# Inference backend (transformers/torch on Windows; mlx_vlm on macOS)
# ---------------------------------------------------------------------------


class ChandraEngine:
    """Loads the model once and serves page inferences (server mode)."""

    def __init__(self, model_dir: str | None, simulate: bool) -> None:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            class LoopStoppingCriteria(StoppingCriteria):
                """Stop on EOS or when the same token chunk repeats back to back."""

                def __call__(self, input_ids, scores, **kwargs) -> bool:
                    row = input_ids[0]
                    if row.numel() and int(row[-1]) in engine._eos_token_ids:
                        return True
                    tokens = row.tolist()
                    for window in LOOP_STOP_WINDOW_SIZES:
                        needed = window * LOOP_STOP_MAX_REPEATS
                        ...
                        if all(
                            ...
                            for index in range(1, LOOP_STOP_MAX_REPEATS)
                        ):
                            return True
                    return False
            ...
            if torch.cuda.is_available():
                # GPU track: 8-bit bitsandbytes quantization (comparable to
                # the 8-bit MLX quantization macOS ships) so the 5.3B model
                # fits a ~8 GB card; fp32 CPU offload absorbs overflow.
                ...
                # CPU fallback: full bf16 weights (slow but faithful).
                model = AutoModelForMultimodalLM.from_pretrained(
                    self.model_dir,
            ...
            if isinstance(generation_eos, int):
                eos_token_ids.append(generation_eos)
            elif isinstance(generation_eos, (list, tuple)):
                eos_token_ids.extend(int(token) for token in generation_eos)
            eos_token_ids = sorted(set(eos_token_ids))

            self._model = model
            self._processor = processor
            self._eos_token_ids = eos_token_ids
            self._stopping_criteria = StoppingCriteriaList([LoopStoppingCriteria()])
            self.loaded = True
        except Exception as error:  # noqa: BLE001 — surfaced to the client
            self.load_error = str(error)
            print(f"chandra-ocr-worker: model load failed: {error}", file=sys.stderr, flush=True)

    def infer_page(self, image_path: Path) -> str:
        if self.simulate:
            return simulated_layout(image_path.name)
        if not self.loaded:
            raise RuntimeError(self.load_error or "model is still loading")
        with self._lock:  # one inference at a time (mirrors the mac worker lock)
            import torch
            ...
            with torch.inference_mode():
                output_ids = self._model.generate(
                    **inputs,
                    ...
                    do_sample=False,
                    eos_token_id=self._eos_token_ids or None,
    ...
    def process_page(
        ...
        provenance = SIMULATION_PAGE_PROVENANCE if self.simulate else PAGE_PROVENANCE
        raw_text = self.infer_page(image_path)
...
# ---------------------------------------------------------------------------
# Loopback HTTP server (client-server contract with the Go backend)
# ---------------------------------------------------------------------------


def serve(host: str, port: int, model_dir: str | None, simulate: bool) -> None:
    from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

    engine = ChandraEngine(model_dir, simulate)

    class Handler(BaseHTTPRequestHandler):
        protocol_version = "HTTP/1.1"

        def log_message(self, format: str, *args: Any) -> None:  # noqa: A002
            sys.stderr.write("worker: " + (format % args) + "\n")
            sys.stderr.flush()
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                image_file = request.get("image_file") or image_path.name
            except Exception as error:  # noqa: BLE001
                self._send_json(400, {"error": f"invalid request: {error}"})
                return
            if not engine.loaded:
                if engine.load_error:
                    self._send_json(500, {"error": engine.load_error})
                else:
                    self._send_json(503, {"error": "model is still loading"})
                return
            try:
                page = engine.process_page(page_number, image_path, image_file)
            except Exception as error:  # noqa: BLE001
    ...
    # Announce readiness to the Go parent (first stdout line).
    print(json.dumps({"port": bound_port}), flush=True)
    try:
...
# ---------------------------------------------------------------------------
# Legacy per-run CLI (identical contract to the macOS worker)
# ---------------------------------------------------------------------------
...
def persist_document_outputs(
    ...
    for page in ordered_pages:
        page_md = page_output_directory / f"page-{int(page['pageNumber']):04d}.md"
        markdown_sections.append(page_md.read_text(encoding="utf-8").strip())
...
def run_legacy() -> None:
    ...
    engine = ChandraEngine(args.model if not simulation else None, simulation)
    engine.load()
    if not engine.loaded:
        raise RuntimeError(engine.load_error or "failed to load Chandra OCR 2")
    ...
    for page_number, image_name in enumerate(args.images, start=1):
        ...
        structured_page = engine.process_page(
            page_number,
            image_path,
            image_path.name,
        )
...
def main() -> None:
    if len(sys.argv) > 1 and sys.argv[1] == "serve":
        parser = argparse.ArgumentParser(description="Chandra OCR 2 inference server")
        ...
        if not args.simulate and not args.model:
            parser.error("--model is required unless --simulate is set")
        serve(args.host, args.port, args.model, args.simulate)
        return
    run_legacy()
