...
> clean installs, and Datalab's Chandra OCR 2 joins as an optional managed
> layout parser. Opening a PDF still never downloads a model or starts extraction:
...
- Added **Chandra OCR 2** (`datalab-to/chandra-ocr-2`) as an optional managed
  local provider, pinned to the `mlx-community/chandra-ocr-2-oQ8` 8-bit MLX
  conversion at one immutable revision with all 9 artifacts pinned by exact
  size and SHA-256.
- Added resumable, byte-counted setup for the approximately 5.16 GB Chandra
  model with the same forced-offline inference, per-page checkpoints, cancel
  and resume, Markdown, structured JSON, and source-box inspection as the
  other managed parsers.
- Chandra emits labeled layout HTML with normalized `data-bbox` anchors; the
  bundled worker parses Caption, Footnote, Equation-Block, List-Group,
  Page-Header, Page-Footer, Image, Section-Header, Table, Text, and related
  labels into typed blocks with normalized source-PDF boxes.
...
Chandra OCR 2 is an upstream model distributed under the OpenRAIL license.
The app links those terms before setup and stores the model separately at
`~/.okra/providers/chandra-ocr-2`. Hardware eligibility matches Dots: Apple
silicon, macOS 14 or later, at least 16 GB unified memory (24 GB recommended),
and at least 8 GB free disk for setup. Completing setup also requires Python
3.10 or later.
...
PDF overlays. Chandra coverage includes the official layout-HTML prompt
contract, labeled-block parsing, nested-block recovery, out-of-range box
clamping, duplicate-tail suppression, plain-text fallback, checkpoint
provenance, and a simulated end-to-end PDF run.
The candidate passes the brand gate, 35 Python tests, 122 Swift tests across
22 suites, six projection safety tests, and a production Swift build. The two
...
The exact pinned Dots and Chandra weights have not yet completed a clean
current-app, multi-page memory/quality run. That remains an explicit release
gate, so RC.5 is a prerelease candidate, not a stable-quality promise. Apple
Vision is the immediate zero-setup fallback, and Baidu remains available for
established local workflows.
...
okraPDF desktop maintainers (`D.6.15`, `D.6.16`, `okra-project/desktop`).
